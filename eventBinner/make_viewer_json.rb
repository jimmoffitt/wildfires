...
TWEET_URL = 'https://twitter.com/general/status/'
...
def getGeoJSON(tweet)
  ...
  properties = {"message" => tweet["message"], "link" => tweet["link"], "tweet" => tweet["tweet"]}
...
def getGeoJSONTweet(tweet,specifyGeoType)
    ...
    tweet_geo['properties']['tweet_url'] = "#{TWEET_URL}#{tweet['tweet_id']}"
...
def getJSONTweet(tweet,specifyGeoType)
  ...
  tweet_geo['tweet_url'] = "#{TWEET_URL}#{tweet['tweet_id']}"
  
  tweet_geo['geo_type'] = 'tweet_geo' unless !specifyGeoType

  if !tweet['expanded_url'].nil? then
		#[] TODO -- code based on single table schema with delimited entities. Need to update to new multi-table schema.
    tweet_geo['media'] = [] #media is an array since there can be multiple links/photos/etc.
    medias = tweet['expanded_url'].split(',')
    medias.each do |media|
...
def getGeoJSONTweetProfile(tweet, specifyGeoType)
  ...
  tweet_profile['properties']['tweet_url'] = "#{TWEET_URL}#{tweet['tweet_id']}"
...
def getJSONTweetProfile(tweet, specifyGeoType)
  ...
  tweet_profile['tweet_url'] = "#{TWEET_URL}#{tweet['tweet_id']}"
...
def add_non_geo_data(interval_data, rs_tweets, key_name, interval)
  ...
  rs_tweets.each do |row|
    ...
    #Add geo metadata.
    tweet['tweet_url'] = "#{TWEET_URL}#{tweet['tweet_id']}"
...
if __FILE__ == $0 #This script code is executed when running this file.
  ...
  #[] TODO: replace this with MySQL gem?
  #Make a database connection.
  oDS = ActiveRecordClient.new
  oDS.get_database_config('./config/config.yaml')
  oDS.establish_connection

  #-------------------------------------------------------------------------------------------------------------------
  #TODO: get via either the CL or config file. 
  params = {}
  params[:start_date] = "2017-12-04 00:00:00"
  params[:end_date] = "2017-12-11 00:00:00"
  params[:interval] = 60 #minutes

  #[] TODO: why are these here? Config it.
  #Profile Geo parameters.
  params[:region] = 'California'
  params[:profile_west] = -119.49
  params[:profile_east] = -116.4
  params[:profile_south] = 33.67
  params[:profile_north] = 34.9
  ...
  #-------------------------------------------------------------------------------------------------------------------
  #Getting geo-tagged tweets WITH Native Media
  #------------------------------------
  key_name = 'geo_taggged_native_media'
  rs_tweets = Activity.connection.select_all(oSQL.get_geo_tagged_tweets_with_native_media(params))
  data_set['stats'][key_name] = rs_tweets.rows.length #Global stats.
  ...
  #-------------------------------------------------------------------------------------------------------------------
  #Getting geo-tagged tweets WITH LINKS
  #------------------------------------
  key_name = 'geo_taggged_media_links'
  rs_tweets = Activity.connection.select_all(oSQL.get_geo_tagged_tweets_with_links(params))
  data_set['stats'][key_name] = rs_tweets.rows.length #Global stats.
  ...
  #-------------------------------------------------------------------------------------------------------------------
  #Getting geo-tagged tweets WITHOUT LINKS
  #---------------------------------------
  key_name = 'geo_taggged_without_media'
  rs_tweets = Activity.connection.select_all(oSQL.get_geo_tagged_tweets_without_links(params))
  data_set['stats'][key_name] = rs_tweets.rows.length #Global stats.
